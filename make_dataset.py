...
for idx, img_path in enumerate(PATH_IMAGES.iterdir()):
    ...
    for sp in tqdm(range(NUM_RANDOM_SAMPLES), ascii=True, desc=f"{img_path} "):
        ...
        img_cp = img.copy()
        random_tags = default_rng().choice([i for i in range(1,len(tags_path)+1)], size=np.random.randint(1, 4, size=(1,)), replace=False)
        ...
        for i, class_number in enumerate(random_tags):
            ...
            tag = Image.open(list(folder_path.iterdir())[random_tag_img-1]).convert("RGBA")
            ...
            x = np.random.randint(low=int(.25*img.width), high=int((1-.25)*img.width))
            y = np.random.randint(low=int(.25*img.height), high=int((1-.25)*img.height))
            
            x_norm = (x + .5*tag.width) / img.width
            y_norm = (y + .5*tag.height) / img.height
            width_norm = size / img.width
            height_norm = size / img.height

            with open(f"{PATH_PROCESSED_LABELS}/image_{OUTPUT_NAME}.txt", "a") as f:
                f.write("{} {} {} {} {}\n".format(class_number, x_norm, y_norm, width_norm, height_norm))
